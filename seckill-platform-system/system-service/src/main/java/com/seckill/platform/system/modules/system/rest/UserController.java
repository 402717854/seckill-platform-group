...
public class UserController {
    ...
    public ResponseEntity<Object> queryUser(UserQueryCriteria criteria, Pageable pageable){
        ...
        // 数据权限
        List<Long> dataScopes = dataService.getDeptIds(userService.findByName(CurrentUserUtils.getCurrentUsername()));
    ...
    public ResponseEntity<Object> centerUser(@Validated(User.Update.class) @RequestBody User resources){
        if(!resources.getId().equals(CurrentUserUtils.getCurrentUserId())){
    ...
    public ResponseEntity<Object> deleteUser(@RequestBody Set<Long> ids){
        for (Long id : ids) {
            Integer currentLevel =  Collections.min(roleService.findByUsersId(CurrentUserUtils.getCurrentUserId()).stream().map(RoleSmallDto::getLevel).collect(Collectors.toList()));
